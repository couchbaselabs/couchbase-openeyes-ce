...
class AdminController extends ModuleAdminController
{
     ...
     * @throws Exception
     */
    public function actionAddDiagnosis($parent_id = null)
    {
        // Handle POST requests for creating a new diagnosis
        if (isset($_POST['disorder_id'])) {
            ...
            if (OphCoTherapyapplication_TherapyDisorder::model()->find($criteria)) {
                ...
                if ($parent) {
                    ...
                    $criteria->condition = 'parent_id = :pid';
                    $criteria->params = array(':pid' => $parent->id);
                }
                
                $maxOrder = OphCoTherapyapplication_TherapyDisorder::model()->find($criteria);
                $therapy_disorder->display_order = ($maxOrder ? $maxOrder->display_order : 0) + 1;
        ...
        $model = new OphCoTherapyapplication_TherapyDisorder();
        
        // Get parent if parent_id is provided
        $parent = null;
        if ($parent_id) {
            $parent = OphCoTherapyapplication_TherapyDisorder::model()->findByPk((int) $parent_id);
            if (!$parent) {
                throw new CHttpException(404, 'Parent diagnosis not found');
            }
        }
        
        $this->render('add_diagnosis', array(
            'model' => $model,
            'parent' => $parent,
            'parent_id' => $parent_id,
            'title' => 'Add Diagnosis',
            'cancel_uri' => '/OphCoTherapyapplication/admin/viewDiagnoses' . ($parent_id ? '?parent_id=' . $parent_id : ''),
        ));
    }
    ...
    public function actionViewTreatments()
    {
        ...
        // Query directly from MariaDB to avoid Couchbase routing issues
        try {
            $sql = 'SELECT * FROM ophcotherapya_treatment ORDER BY id DESC';
            ...
            Yii::log('Error querying treatments: ' . $e->getMessage(), 'error');
            $models = array();
        }
    ...
    public function actionAddTreatment()
    {
        ...
        if (isset($_POST['OphCoTherapyapplication_Treatment'])) {
            ...
            if ($model->save()) {
                ...
                Yii::app()->user->setFlash('success', 'Treatment created with ID: ' . $model->id);

                $this->redirect(array('viewTreatments'));
            } else {
                // Log errors if save failed
                Yii::log('Treatment save failed: ' . print_r($model->getErrors(), true), 'error');
                Yii::app()->user->setFlash('error', 'Failed to create treatment: ' . implode(', ', array_values(array_filter(array_map(function($e) { return implode(', ', $e); }, $model->getErrors())))));
            }
        }
    ...
    public function actionViewEmailRecipients()
    {
        ...
        $criteria = new CDbCriteria();
        $criteria->with = array('site' => array('joinType' => 'LEFT OUTER JOIN'));
        $criteria->condition = 't.institution_id = :institution_id OR t.site_id is null OR site.institution_id = :institution_id';
        $criteria->params = array(':institution_id' => Yii::app()->session['selected_institution_id']);
        $criteria->order = 'display_order asc';

        $this->render('list_OphCoTherapyapplication_Email_Recipient', array(
                'model_class' => 'OphCoTherapyapplication_Email_Recipient',
                'model_list' => OphCoTherapyapplication_Email_Recipient::model()->findAll($criteria),
