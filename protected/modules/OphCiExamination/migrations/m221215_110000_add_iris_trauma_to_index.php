...
class m221215_110000_add_iris_trauma_to_index extends OEMigration
{
    ...
    {
        $this->execute("INSERT IGNORE INTO eyedraw_doodle
        ...
        $this->insertCanvasMappingIfPossible('EXAM_ANT_SEG');
        $this->insertCanvasMappingIfPossible('OP_NOTE_CAT_ANT_SEG');
        ...
        if (!$exam_id || !$parent_id) {
            $this->migrationEcho('Skipping index_search inserts for Iris Trauma - required reference data missing.');
            return true;
        }
        $this->execute(
    ...
    private function insertCanvasMappingIfPossible(string $canvasMnemonic): void
    {
        $canvasExists = (bool) $this->dbConnection->createCommand(
            'SELECT 1 FROM eyedraw_canvas WHERE canvas_mnemonic = :mnemonic'
        )->queryScalar([':mnemonic' => $canvasMnemonic]);

        if (!$canvasExists) {
            $this->migrationEcho("Skipping IrisTrauma mapping for canvas {$canvasMnemonic} - canvas not found.");
            return;
        }

        $this->execute("INSERT IGNORE INTO eyedraw_canvas_doodle
                        ...
                        VALUES('IrisTrauma', :canvas, '', 0, 0, 1, 0);
        ", [':canvas' => $canvasMnemonic]);
    }
}
