...
class DefaultController extends OphTrOperationbookingEventController
{
    ...
    protected function initWithEventId($id)
    {
        ...
        if ($this->operation_required && !$this->operation) {
            if (!$this->eur_res || !$this->eur_res->id) {
    ...
    public function actionCreate()
    {
        ...
        $setting = SettingMetadata::model()->findByAttributes(array('key' => 'require_exam_before_booking'));
        $require_exam_before_booking = $setting ? $setting->getSettingName() : 'off';
    ...
    {
        // Print actions handle their own initialization via printInit()
        // Set operation_required flag for validation, but dont initialize here
        $this->operation_required = true;
    }
     ...
     * @throws Exception
     */
    public function actionAdmissionLetter($id = null)
    {
        // Handle case where ID is not provided in action parameter
        if (!$id) {
            $id = Yii::app()->request->getParam('id');
    ...
    {
        $id_param = Yii::app()->request->getParam('id');
        if (!$id_param) {
            $this->renderJSON(null);
            return;
        }
        
        $procedure_ids = array_map('intval', explode(',', $id_param));
        $criteria = new \CDbCriteria();
