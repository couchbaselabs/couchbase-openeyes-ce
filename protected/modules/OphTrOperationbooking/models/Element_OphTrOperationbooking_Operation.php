...
class Element_OphTrOperationbooking_Operation extends BaseEventTypeElement
{
    ...
    // Store pending procedures and anaesthetic type assignments for new records
    // This works around a Yii1 issue where relation properties are overwritten during validation
    protected $_pendingProcedures = null;
    protected $_pendingAnaestheticTypeAssignments = null;
    ...
    public function init()
    {
        $this->anaesthetist_required_ids = array(
            Yii::app()->cbdb->createCommand()->select('id')->from('anaesthetic_type')->where('code=:code', array(':code' => 'Sed'))->queryScalar(),
            Yii::app()->cbdb->createCommand()->select('id')->from('anaesthetic_type')->where('code=:code', array(':code' => 'GA'))->queryScalar(),
        );
    ...
    /**
     * Override __get to handle procedures and anaesthetic_type_assignments for new records.
     * This works around a Yii1 issue where the 'required' validator triggers relation loading
     * which overwrites manually set relation values.
     * Also handles hydration of embedded Couchbase data back to model objects.
     */
    public function __get($name)
    {
        // Only use pending values for new records that haven't been saved yet
        if ($this->getIsNewRecord()) {
            if ($name === 'procedures' && $this->_pendingProcedures !== null) {
                return $this->_pendingProcedures;
            }
            if ($name === 'anaesthetic_type_assignments' && $this->_pendingAnaestheticTypeAssignments !== null) {
                return $this->_pendingAnaestheticTypeAssignments;
            }
        }
        ...
        if ($name === 'procedures' && is_array($value) && !empty($value)) {
            ...
            if (is_array($firstItem) && isset($firstItem['id'])) {
                // Hydrate embedded procedure arrays back to Procedure models
                $hydratedProcedures = [];
                foreach ($value as $procData) {
                    if (is_array($procData) && isset($procData['id'])) {
                        $proc = \Procedure::model()->findByPk($procData['id']);
                        if ($proc) {
                            $hydratedProcedures[] = $proc;
                        }
                    } elseif ($procData instanceof \Procedure) {
                        $hydratedProcedures[] = $procData;
                    }
                }
                return $hydratedProcedures;
            }
        }
    ...
    /**
     * Override __set to store procedures and anaesthetic_type_assignments for new records.
     */
    public function __set($name, $value)
    {
        if ($this->getIsNewRecord()) {
            if ($name === 'procedures') {
                $this->_pendingProcedures = $value;
                return;
            }
            if ($name === 'anaesthetic_type_assignments') {
                $this->_pendingAnaestheticTypeAssignments = $value;
                return;
            }
        }
    ...
    {
        if ($this->event && $this->event->episode && $this->event->episode->firm) {
    ...
    {
        // Handle case where event relation might not be loaded (e.g., from Couchbase)
        $createdDate = null;
        if ($this->event) {
            $createdDate = $this->event->created_date;
        } elseif ($this->event_id) {
            // Try to load event directly if relation not available
        ...
        $date = $createdDate ? strtotime($createdDate) : time();
                ...
                if (date('D', mktime(0, 0, 0, $month, $i, $year)) == $day) {
                    ...
                    if (in_array($date, $dates) && (Yii::app()->user->checkAccess('Super schedule operation') || !SettingMetadata::model()->getSetting('future_scheduling_limit') || $date <= date('Y-m-d', strtotime('+' . SettingMetadata::model()->getSetting('future_scheduling_limit'))))) {
                        ...
                        if (strtotime($date) < strtotime(date('Y-m-d'))) {
                            ...
                            if ($schedule_options && !$schedule_options->isPatientAvailable($date)) {
    ...
    public function getTheatres($date, $firm_id = false)
    {
        ...
        // Check if we're in Couchbase-first mode (MariaDB unavailable)
        $mariaDbUnavailable = false;
        try {
            $db = Yii::app()->db;
            if ($db instanceof \OEDbConnection) {
                $mariaDbUnavailable = !$db->isConnectionAvailable();
            } else {
                $pdo = $db->getPdoInstance();
                $mariaDbUnavailable = ($pdo === null);
            }
        } catch (\Throwable $e) {
            $mariaDbUnavailable = true;
        }

        if ($mariaDbUnavailable) {
            // Load sessions directly from Couchbase and group by theatre
            $sessionCriteria = new CDbCriteria();
            $sessionCriteria->compare('date', $date);
            $sessionCriteria->compare('available', 1);
            if (empty($firm_id) || $firm_id == 'EMG') {
                $sessionCriteria->addCondition('firm_id IS NULL');
            } else {
                $sessionCriteria->compare('firm_id', $firm_id);
            }
            $sessionCriteria->order = 'start_time ASC';

            $sessions = OphTrOperationbooking_Operation_Session::model()->findAll($sessionCriteria);

            // Group sessions by theatre and load theatres
            $theatreIds = array();
            $sessionsByTheatre = array();
            foreach ($sessions as $session) {
                $theatreIds[$session->theatre_id] = $session->theatre_id;
                if (!isset($sessionsByTheatre[$session->theatre_id])) {
                    $sessionsByTheatre[$session->theatre_id] = array();
                }
                $sessionsByTheatre[$session->theatre_id][] = $session;
            }
            ...
            // Load theatres
            $theatreCriteria = new CDbCriteria();
            $theatreCriteria->addInCondition('id', array_values($theatreIds));
            $theatres = OphTrOperationbooking_Operation_Theatre::model()->active()->findAll($theatreCriteria);

            // Attach sessions to theatres using setCachedSessions
            foreach ($theatres as $theatre) {
                $theatre->setCachedSessions(isset($sessionsByTheatre[$theatre->id]) ? $sessionsByTheatre[$theatre->id] : array());
            }

            return $theatres;
        }

        // Standard MySQL/MariaDB mode with eager loading
        $criteria = new CDbCriteria();
    ...
    public function getWardOptions($session)
    {
        ...
        // Fallback: load theatre directly if relation returns null
        if (!$theatre && $theatreId) {
            $theatre = OphTrOperationbooking_Operation_Theatre::model()->findByPk($theatreId);
        }
        
        $siteId = $theatre ? $theatre->site_id : null;

        $results = array();

        if (!empty($theatreId) && $theatre) {
            if ($theatre->ward) {
                $results[$theatre->ward_id] = $theatre->ward->name;
            }
        }
        ...
        if (empty($results)) {
            ...
            // For Couchbase compatibility, use simpler condition for unrestricted wards
            // Bitwise operations may not work the same way
            if ($genderRestrict == 0 && $ageRestrict == 0) {
                // No patient or restrictions - just get unrestricted wards
                $criteria->compare('restriction', 0);
            } else {
                // With patient restrictions - try bitwise check, fallback to unrestricted
                $criteria->addCondition('(restriction & :r1 > 0 AND restriction & :r2 > 0) OR restriction = 0');
    ...
    public function calculateEROD($firm = null)
    {
        ...
        // Get the firm - either from the operation or try to load via event
        $operationFirm = $this->getFirm();
        if (!$operationFirm && $this->event_id) {
            // Try to load event/episode/firm chain directly if relations not loaded
            $event = \Event::model()->findByPk($this->event_id);
            if ($event && $event->episode && $event->episode->firm) {
                $operationFirm = $event->episode->firm;
            }
        }
        
        // If we still don't have a firm, we can't calculate EROD by subspecialty rule
        $rule = null;
        if ($operationFirm) {
            $rule = OphTrOperationbooking_Operation_EROD_Rule::model()->find('subspecialty_id=?', array($operationFirm->getSubspecialtyID()));
        }
        ...
        if ($rule) {
            ...
            if (!$firm) {
                // Try to get firm from relations or loaded event
                if ($this->event && $this->event->episode && $this->event->episode->firm) {
                    $firm = $this->event->episode->firm;
                } elseif ($operationFirm) {
                    $firm = $operationFirm;
                } else {
                    // Can't calculate EROD without a firm
                    return null;
                }
            }
            ...
            if (!$firm->id) {
                ...
                $subspecialty = Subspecialty::model()->find('ref_spec=?', array('AE'));
                if (!$subspecialty) {
                    // A&E subspecialty not configured - return null to skip EROD calculation
                    return null;
                }

                $service_subspecialty_assignment = ServiceSubspecialtyAssignment::model()->find('subspecialty_id=?', array($subspecialty->id));
                if (!$service_subspecialty_assignment) {
                    // A&E service_subspecialty_assignment not configured - return null
                    return null;
                }
    ...
    public function audit($target, $action, $data = null, $log = false, $properties = array())
    {
        ...
        $properties['episode_id'] = $this->event ? $this->event->episode_id : null;
        
        // Safely get patient_id with fallback
        $patient = $this->getPatient();
        $properties['patient_id'] = $patient ? $patient->id : null;
    ...
    public function cancel($reason_id, $comment = null, $cancellation_user_id = false, $reschedule_later = false)
    {
        ...
        // Fallback: load episode directly if relation returns null
        if (!$episode && $this->event && $this->event->episode_id) {
    ...
    {
        if (!$this->status) {
            return true; // Default to editable if status not set
        }
    ...
    public function schedule(
        ...
        // Skip validation when scheduling - the procedures/consultant relations may not load
        // correctly from Couchbase, but the data is already validated at creation time
        if (!$this->save(false)) {
        ...
        // Skip validation for session save
        ...
        if ($booking->session && $booking->session->firm && $erod = $this->calculateEROD($booking->session->firm)) {
        ...
        // Load episode with fallback for Couchbase mode
        $episode = $this->event ? $this->event->episode : null;
        if (!$episode && $this->event && $this->event->episode_id) {
            $episode = Episode::model()->findByPk($this->event->episode_id);
        }
        if ($episode) {
            ...
            if (!$episode->save(false)) {
                throw new Exception('Unable to change episode status id for episode ' . $episode->id);
            }
        }

        // Delete issues from event - handle null event relation from Couchbase
        $event = $this->event;
        if (!$event && $this->event_id) {
            $event = \Event::model()->findByPk($this->event_id);
        }
        if ($event) {
            $event->deleteIssues();
    ...
    public function getComplexityCaption()
    {
        ...
        // Handle Couchbase format where complexity is stored as object {"caption": "Low", "value": 0}
        if (is_array($complexity) && isset($complexity['caption'])) {
            return $complexity['caption'];
        }
        if (is_object($complexity) && isset($complexity->caption)) {
            return $complexity->caption;
        }
        // Handle standard integer format
        if (is_numeric($complexity) && array_key_exists((int)$complexity, self::$complexity_captions)) {
            return self::$complexity_captions[(int)$complexity];
        }
        return 'N/A';
    }
    ...
    public function getComplexityColor()
    {
        $complexity = $this->complexity;
        // Handle Couchbase format where complexity is stored as object {"caption": "Low", "value": 0}
        if (is_array($complexity) && isset($complexity['value'])) {
            $complexity = $complexity['value'];
        }
        if (is_object($complexity) && isset($complexity->value)) {
            $complexity = $complexity->value;
        }
        if (is_numeric($complexity) && array_key_exists((int)$complexity, self::$complexity_colors)) {
            return self::$complexity_colors[(int)$complexity];
        }
        return '';
    }
    ...
    {
        // Clear pending values after save so the database relation is used for subsequent access
        $this->_pendingProcedures = null;
        $this->_pendingAnaestheticTypeAssignments = null;
    ...
    protected function getEmbeddedRelations()
    {
        ...
        // Note: $this->procedures is a MANY_MANY relation returning Procedure models directly
        if (!empty($this->procedures)) {
            $data['procedures'] = array_map(function($proc) {
                return [
                    'id' => (int)$proc->id,
                    'term' => $proc->term ?? null,
                    'short_format' => $proc->short_format ?? null,
                    'snomed_code' => $proc->snomed_code ?? null,
                    'default_duration' => isset($proc->default_duration) ? (int)$proc->default_duration : null,
                ];
            }, $this->procedures);
        }
