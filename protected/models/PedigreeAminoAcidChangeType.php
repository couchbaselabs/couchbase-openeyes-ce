...
class PedigreeAminoAcidChangeType extends BaseActiveRecord
{
     ...
     * Always save directly to Couchbase since MariaDB is deprecated
     */
    public function save($runValidation = true, $attributes = null, $allow_overriding = false)
    {
        // Always save to Couchbase directly for this model
        // MariaDB is deprecated and may not be available
        if ($this->getIsNewRecord() || !isset($this->id)) {
            $this->created_user_id = Yii::app()->user->id ?? 1;
            $this->created_date = date('Y-m-d H:i:s');
            // Generate a unique ID for Couchbase storage
        ...
        // Save directly to Couchbase using REST API (bypasses SDK crash on ARM64)
        try {
            $restClient = Yii::app()->couchbaseRest;
            $doc = $this->toCouchbaseDocument();
            $docId = $this->couchbaseCollection() . '::' . $this->id;
            $restClient->upsert($this->couchbaseScope(), $this->couchbaseCollection(), $docId, $doc);
            return true;
        } catch (Exception $e) {
            Yii::log('Couchbase save failed for PedigreeAminoAcidChangeType: ' . $e->getMessage(), CLogger::LEVEL_ERROR, 'application.couchbase');
            return false;
        }
    }
    ...
    /**
     * Override isDualWriteEnabled to always enable Couchbase writes for this model
     * since MariaDB is deprecated
     */
