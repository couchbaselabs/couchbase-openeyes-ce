...
class WorklistController extends BaseController
{
    ...
    public function actionGetPathStep($partial, $pathstep_id, $visit_id, $pathstep_type_id, $red_flag = false, $interactive = 1)
    {
        ...
        switch ($pathstep_id) {
            ...
            default:
                ...
                if (
                    ($step instanceof PathwayStep && $step->type && $step->type->short_name === 'drug admin')
                    || ($step instanceof PathwayTypeStep && $step->step_type && $step->step_type->short_name === 'drug admin')
                ) {
                    if (!$wl_patient) {
                        throw new CHttpException('Unable to retrieve pathway for patient.');
                    }
                ...
                if (
                    ($step instanceof PathwayStep && $step->type && $step->type->short_name === 'Rx')
                    || ($step instanceof PathwayTypeStep && $step->step_type && $step->step_type->short_name === 'Rx')
                ) {
                    $has_permission_to_start = Yii::app()->user->checkAccess('TaskPrescribe');
                }
                if ($step && $wl_patient) {
                    $view_file = $red_flag ? 'generic_step' :
                        ($step instanceof PathwayStep ? ($step->type ? $step->type->widget_view : null) : ($step->step_type ? $step->step_type->widget_view : null)) ?? 'generic_step';
                    $dom = $this->renderPartial(
                        '//worklist/steps/' . $view_file,
                        array(
                            'step' => $step,
    ...
    public function actionRenderPopup()
    {
        if (isset($_POST['patientId'])) {
            ...
            if (!$patient) {
                $this->renderJSON(['error' => 'Patient not found']);
                return;
            }
    ...
    public function actionAssignUserToPathway()
    {
        ...
        if (empty($id) || empty($visit_id)) {
            throw new CHttpException(400, 'Missing required parameters: user_id and target_visit_id must be provided.');
        }
